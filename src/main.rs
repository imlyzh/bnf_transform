...
use crate::program::*;
use crate::optimize::{LeftRecOptimize, OneOrMoreOptimize};
...
fn main() {
    ...
    if args.get(1).unwrap() == "--golang" {
        ...
        write!(w, "{}", golang::gen::Gen::gen(&res.left_rec_optimize().one_or_more_optimize())).expect("write error");
    } else if args.get(1).unwrap() == "--llir" {
        let res = llir::parser::parse(&buf).expect("is not valid bnf format");
        let mut w = File::create(args.get(3).unwrap()).expect("output file open invalid");
        write!(w, "{}", llir::gen::Gen::gen(&res.left_rec_optimize().one_or_more_optimize())).expect("write error");
    } else {
